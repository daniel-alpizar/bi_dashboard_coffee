...
def app_layout_refresh():
    ...
    last_thu = Database.objects.aggregate(Max('Fecha')).get('Fecha__max')
    last_thu2 = last_thu - timedelta(7)

    database = Database.objects.filter(Fecha__range=(last_thu2, last_thu))
...
def update_graph(drop_actividad):
    ...
    fecha_max = Database.objects.aggregate(Max('Fecha')).get('Fecha__max')
    database = Database.objects.filter(Fecha=fecha_max)
    df = pd.DataFrame.from_records(database.values())
    df = df[~(df.Familia.isin(['Produccion', 'Cosecha']))]   # Excluir ingresos y costos de cosecha
    df = df[~(df.Labor.isin(['0']))]   # Eliminar Costos/Hrs huerfanas de labores

    df_wk = df.groupby(['Familia', 'Actividad', 'Labor']).agg(IN_Uds=('IN_Uds', sum), MO_Hrs=('MO_Hrs',
                                    sum), MAQ_Hrs=('MAQ_Hrs', sum), Tot_CRC=('Tot_CRC', sum), Fecha=('Fecha', max),
                                    Diesel=('Diesel', sum), Gas=('Gas', sum), Area=('Area', sum))
    df_wk.reset_index(inplace=True)
...
def update_graph(drop_actividad):
    ...
    df1 = df1.groupby(['Parcela', 'Actividad']).agg(MO_Hrs=('MO_Hrs', sum),
